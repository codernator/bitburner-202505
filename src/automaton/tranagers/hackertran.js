...
export async function main(ns) {
  ...
  const underAttack = StateMan.deserialize(state);
  const hackable = getHackable(allservers, hackingLevel)
    .filter(tpl => !underAttack.has(StateMan.createEntry(tpl.server.hostname, tpl.attackMode)));
  ...
  for (let { server, attackMode } of hackable) {
    ...
    ns.writePort(queuePort, `${targetname}:${attackMode}`);
